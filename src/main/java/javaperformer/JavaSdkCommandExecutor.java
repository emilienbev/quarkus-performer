...
import core.perf.PerRun;
import core.stream.StreamStreamer;
import core.util.ErrorUtil;
...
import javaperformer.eventing.EventingHelper;
import javaperformer.manager.BucketManagerHelper;
import javaperformer.manager.CollectionManagerHelper;
import javaperformer.query.QueryIndexManagerHelper;
import javaperformer.search.SearchHelper;
import javaperformer.utils.ClusterConnection;
import javaperformer.utils.ContentAsUtil;
...
import static core.util.TimeUtil.getTimeNow;
...
import static javaperformer.search.SearchHelper.handleSearchBlocking;
import static javaperformer.search.SearchHelper.handleSearchQueryBlocking;
...
public class JavaSdkCommandExecutor extends SdkCommandExecutor {
    ...
    @Override
    protected void performOperation(Command op, PerRun perRun) {
        var result = performOperationInternal(op, perRun);
        perRun.resultsStream().enqueue(result);
    }

    protected Result performOperationInternal(Command op, PerRun perRun) {
  ...
  public static MutateInSpec convertMutateInSpec(com.couchbase.client.protocol.sdk.collection.mutatein.MutateInSpec requestSpec) {
    ...
    public static Optional<ScanTerm> convertScanTerm(com.couchbase.client.protocol.sdk.kv.rangescan.ScanTermChoice st) {
        if (st.hasDefault()) {
            ...
            if (stt.hasExclusive() && stt.getExclusive()) {
                if (stt.hasAsString()) {
                    return Optional.of(ScanTerm.exclusive(stt.getAsString()));
                }
                else throw new UnsupportedOperationException();
            }
            if (stt.hasAsString()) {
                return Optional.of(ScanTerm.inclusive(stt.getAsString()));
            }
            else throw new UnsupportedOperationException();
        }
        else throw new UnsupportedOperationException();
    ...
    public static ScanType convertScanType(Scan request) {
        if (request.getScanType().hasRange()) {
            ...
            if (rs.hasFromTo()) {
                ...
                if (from.isPresent() && to.isPresent()) {
                    return ScanType.rangeScan(from.get(), to.get());
                }
                else if (from.isPresent()) {
                    return ScanType.rangeScan(from.get(), null);
                }
                else if (to.isPresent()) {
                    return ScanType.rangeScan(null, to.get());
                }
                else {
                ...
                return ScanType.prefixScan(rs.getDocIdPrefix());
            }
            else throw new UnsupportedOperationException();
        }
            ...
            if (ss.hasSeed()) {
                return ScanType.samplingScan(ss.getLimit(), ss.getSeed());
            }
            return ScanType.samplingScan(ss.getLimit());
        }
        else {
    ...
    public static void setSuccess(Result.Builder result) {
        result.setSdk(com.couchbase.client.protocol.sdk.Result.newBuilder()
                .setSuccess(true));
    }

    public static void populateResult(Result.Builder result, MutationResult value) {
  ...
  public static void populateResult(ContentAs contentAs, Result.Builder result, GetResult value) {
    ...
    public static void populateResult(Result.Builder result, MutateInResult value, MutateIn request) {
    ...
    public static void populateResult(Result.Builder result, CounterResult value) {
    ...
    public static void populateResult(Result.Builder result, GetReplicaResult value, ContentAs contentAs) {
        var content = ContentAsUtil.contentType(contentAs,
                () -> value.contentAs(byte[].class),
                () -> value.contentAs(String.class),
                () -> value.contentAs(JsonObject.class),
    ...
    public static void populateResult(Result.Builder result, ExistsResult value) {
    ...
    public static void populateResult(com.couchbase.client.protocol.sdk.query.Command request, Result.Builder result, QueryResult values) {
    ...
    // [if:3.4.1]
    public static @Nullable ScanOptions createOptions(Scan request, ConcurrentHashMap<String, RequestSpan> spans) {
        if (request.hasOptions()) {
            var opts = request.getOptions();
    ...
    public static @Nullable GetAllReplicasOptions createOptions(GetAllReplicas request) {
        if (request.hasOptions()) {
            var opts = request.getOptions();
    ...
    public static @Nullable MutateInOptions createOptions(MutateIn request) {
        if (request.hasOptions()) {
            var opts = request.getOptions();
    ...
    public static void convertDurability(DurabilityType durability, CommonDurabilityOptions options) {
    ...
    public static MutationState convertMutationState(com.couchbase.client.protocol.shared.MutationState consistentWith) {
        var mutationTokens = consistentWith.getTokensList().stream()
                .map(mt -> new MutationToken((short) mt.getPartitionId(),
                        mt.getPartitionUuid(),
                        mt.getSequenceNumber(),
                        mt.getBucketName()))
                .toList();
        return MutationState.from(mutationTokens.toArray(new MutationToken[0]));
    }
  ...
  public static WaitUntilReadyOptions waitUntilReadyOptions(WaitUntilReadyRequest request) {
    ...
    if (request.getOptions().getServiceTypesList().size() > 0) {
      ...
      for (com.couchbase.client.protocol.sdk.cluster.waituntilready.ServiceType service : serviceTypes) {
        var newService = ServiceType.valueOf(service.toString());
